...
export async function resolvePhotoUrl(
  ...
  const resolveUrl = await Promise.all(
  ...
  return resolveUrl.filter((url): url is string => url !== null);
}
...
export async function withResolvedPhotos<T extends Doc<"users"> | null>(
  ...
  const resolvedPhotos = await resolvePhotoUrl(ctx, user.photos);
...
export async function withResolvedPhotosArray(
  ...
  return Promise.all(
    users.map((user) => withResolvedPhotos(ctx, user)),
  ) as Promise<Doc<"users">[]>;
}
